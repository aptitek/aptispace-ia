::: {.dynamic .horizontal .side-results .has-controls header="Le Mur de la Sparsité"}
...
html`<div style="display: flex; gap: 40px; align-items: center; width: 100%; justify-content: center;">
...
viewof monitor_sparsity = {
  ...
  m.dist = m.addPieChart("Distribution des Bits (0 vs 1)", [
    { name: "Zéros", value: bitCounts.zeros, color: theme.green },
    { name: "Uns", value: bitCounts.ones, color: theme.red }
  ]);
  return m;
}
```
...
// Logic for real-time updates
{
  ...
  if (monitor) {
    ...
    if (monitor.dist) monitor.dist.update([
      { name: "Zéros", value: (1 - displayOnes) * 100, color: theme.green },
      { name: "Uns", value: displayOnes * 100, color: theme.red }
    ]);
  }
}
```
