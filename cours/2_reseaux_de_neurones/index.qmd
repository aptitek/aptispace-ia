...
```{ojs}
//| echo: false
viewof hyper_W = Inputs.range([10, 227], {value: 227, step: 1, label: "Entrée (W)"})
viewof hyper_F = Inputs.range([1, 11], {value: 11, step: 1, label: "Filtre (F)"})
viewof hyper_P = Inputs.range([0, 5], {value: 0, step: 1, label: "Padding (P)"})
viewof hyper_S = Inputs.range([1, 5], {value: 4, step: 1, label: "Stride (S)"})
...
```{ojs}
//| echo: false
viewof drp_rate = Inputs.range([0, 0.9], {value: 0.6, step: 0.01, label: "Taux de Dropout :"})
...
::: {.controls header="Transformations"}
```{ojs}
//| echo: false
viewof augmentation = Inputs.form({
  rot: Inputs.toggle({label: "🔄 Rotation"}),
  flip: Inputs.toggle({label: "↔️ Effet Miroir"}),
  zoom: Inputs.toggle({label: "🔍 Zoom"})
}, {
  template: (inputs) => html`<div class="augmentation-controls-column">${Object.values(inputs)}</div>`
})

do_rot = augmentation.rot;
do_flip = augmentation.flip;
do_zoom = augmentation.zoom;
```
:::
